/*
  ...
  AUR4 Clock
  RTC clock using NTP server to sync time - https://github.com/eremef/aur4_clock
...
v.2.0 - Nicu FLORICA (niq_ro) updated the second beats as at https://www.hackster.io/mdraber/digital-clock-with-arduino-uno-rev4-wifi-s-rtc-an-led-matrix-484d2e
        and changed the number style
*/
...
position secunda = {2, 5}; // position for seconds
position first1  = {5, 0}; // position of first digit
position second2 = {0, 0}; // etc.
position third3  = {5, 7};
position fourth4 = {0, 7};
...
void loop() {  
  ...
  //int secunds = currentTime.getSeconds();
  byte secunds = millis()/1000;
  Serial.println(secunds);
  
//  matrix.clear();
  if (secunds%2 != 0)
      setDigit(secunda, digits[10]);
  setDigit(first1, digits[hour.substring(0,1).toInt()]);
  setDigit(second2, digits[hour.substring(1).toInt()]);
  setDigit(third3, digits[minutes.substring(0,1).toInt()]);
  setDigit(fourth4, digits[minutes.substring(1).toInt()]);
  if (ORIENTATION == 1)
   {
    rotateFrame();
   } 
